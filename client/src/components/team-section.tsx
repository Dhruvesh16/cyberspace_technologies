      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <motion.h2 className="text-4xl md:text-5xl font-bold text-white mb-4">
            {isHomePage ? "Meet Our" : "Our"} <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Team</span>
          </motion.h2>
        ...
        <div className={`grid ${isHomePage ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'} gap-8`}>
          {displayedMembers.map((member, index) => (
            ...
            >
              <motion.div 
                className="bg-white/95 backdrop-blur-sm border-2 border-purple-500/30 rounded-2xl p-6 text-center group-hover:shadow-2xl group-hover:shadow-purple-500/30 transition-all duration-500"
              ...
              >
                ...
                <motion.div className="mb-4">
                  <motion.h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-purple-600 transition-colors duration-300">
                    {member.name}
                  </motion.h3>
                  <motion.p className={`text-sm font-medium text-purple-600 mb-3`}>
                    {member.role}
                  </motion.p>
                </motion.div>

                <motion.p className="text-gray-700 text-sm mb-6 group-hover:text-gray-800 transition-colors duration-300">
                  {member.bio}
                </motion.p>
              </motion.div>
            </motion.div>
        ...
        {isHomePage && (
          ...
          >
            <Link href="/team">
              <Button 
                size="lg" 
                className="px-8 py-4 rounded-2xl shadow-xl hover:shadow-2xl font-semibold text-lg group bg-gradient-to-r from-purple-600 to-pink-600 hover:from-pink-600 hover:to-purple-600 transition-all duration-500 text-white border-0 transform hover:scale-105"
                style={{
                  background: "linear-gradient(135deg, #9333ea 0%, #ec4899 100%)",
                  boxShadow: "0 10px 25px rgba(147, 51, 234, 0.4), 0 0 0 1px rgba(255, 255, 255, 0.1)"
                }}
              >
